...
pick_pose_base = trvec2tform([0.15, 0.15, 0.05]);  % Default pick pose ( You can change it)
...
function main_task()
    ...
    while true
        ...
        %joint_positions=[0,-1,1,0];
        disp("current joint angles");disp(joint_positions);
        disp("Pick pose");disp(pick_pose_base);
        weights=[1e7,1e8,1e8,1e8,1e8,1e8]
        % Compute Forward Kinematics for the current joint positions
        T_eef = fKpincherx100(joint_positions);
        initial_guess=[0.2084  ,  0.0912   , 0.2642,   -0.3554,0.3526,    0.1544  ,  0.4470 ];
        % Compute the desired joint angles for pick position
        %pick_joint_angles = iKpincherx100(pick_pose_base, "up");
        ik=inverseKinematics("RigidBodyTree",Px100.robot,"SolverAlgorithm","LevenbergMarquardt");
        %ik.MaxIterations=2000;
        %ik.SolutionTolerance=1e-7;

         [config,sols]=ik( "px100/ee_arm_link",pick_pose_base,weights,initial_guess)
         pick_joint_angles=config(1:4);
    ...
    % %% Transporting Payload
      while true
         initial_guess=Px100.homeConfig
         ...
         [rconfig,rsols]=ik( "px100/ee_arm_link",drop_pose_base,weights,initial_guess)
         drop_joint_angles=rconfig(1:4);
         disp("Dropping the care package")
         ...
         joint_space_waypoints = generate_waypoints(joint_positions, drop_joint_angles, num_waypoints)
         % Execute trajectory
         execute_trajectory(joint_space_waypoints);
